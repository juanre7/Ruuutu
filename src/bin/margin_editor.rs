...
use font::{Canvas, draw_consolas_bold_text, draw_consolas_bold_text_clipped, draw_svg_icon, measure_consolas_bold_width, IconType};
        ...
        if let Event::WindowEvent { event, .. } = event { match event {
                        ...
                        if cat.expanded {
                            for (p_idx, p) in cat.props.iter_mut().enumerate() {
                                if cy >= curr_y && cy < curr_y + item_h {
                                    ...
                                    if (440..=480).contains(&cx) {
                                        let mult = if shift_pressed { 5.0 } else { 1.0 };
                                        p.val = (p.val - p.step * mult).clamp(p.min, p.max);
                                    } else if (490..=530).contains(&cx) {
                                        let mult = if shift_pressed { 5.0 } else { 1.0 };
                                        p.val = (p.val + p.step * mult).clamp(p.min, p.max);
                                    }
                                    window.request_redraw();
            ...
            WindowEvent::RedrawRequested => {
                ...
                draw_consolas_bold_text(
                    &mut canvas, "RUUUTU - STUDIO EDITOR (CUADRADOS PERFECTOS, CENTRADO & DELAY MS)",
                    24, 16, 0x38BDF8, 19.0,
                );
                draw_consolas_bold_text(
                    &mut canvas, "Botones cerrados = Cuadrados perfectos con icono centrado. Expansión fluida tras delay.",
                    24, 42, 0x94A3B8, 12.5,
                );
                ...
                fill_rect(&mut canvas, 15, 70, panel_w, buf_h - 130, 0x1E293B);
                draw_border(&mut canvas, 15, 70, panel_w, buf_h - 130, 0x334155);
                ...
                for (c_idx, cat) in categories.iter().enumerate() {
                    ...
                    fill_rect(&mut canvas, 20, render_y, panel_w - 10, 30, header_bg);

                    let arrow = if cat.expanded { "▼ " } else { "► " };
                    let header_text = format!("{}{}", arrow, cat.name);
                    draw_consolas_bold_text(&mut canvas, &header_text, 28, render_y + 7, 0xFFFFFF, 13.5);
                    ...
                    if cat.expanded {
                        for (p_idx, p) in cat.props.iter().enumerate() {
                            ...
                            fill_rect(&mut canvas, 25, render_y, panel_w - 20, 32, row_bg);

                            let label_col = if is_prop_sel { 0xFACC15 } else { 0xE2E8F0 };
                            draw_consolas_bold_text(&mut canvas, p.name, 35, render_y + 8, label_col, 12.5);

                            let val_str = if p.step < 1.0 { format!("{:.2}{}", p.val, p.unit) } else { format!("{:.0}{}", p.val, p.unit) };
                            draw_consolas_bold_text(&mut canvas, &val_str, 355, render_y + 8, 0x38BDF8, 12.5);

                            fill_rect(&mut canvas, 440, render_y + 4, 34, 24, 0x334155);
                            draw_consolas_bold_text(&mut canvas, "-", 453, render_y + 7, 0xFFFFFF, 14.0);

                            fill_rect(&mut canvas, 485, render_y + 4, 34, 24, 0x334155);
                            draw_consolas_bold_text(&mut canvas, "+", 497, render_y + 7, 0xFFFFFF, 14.0);
                ...
                fill_rect(&mut canvas, prev_x, prev_y, prev_w, prev_h, 0x020617);
                draw_border(&mut canvas, prev_x, prev_y, prev_w, prev_h, 0x1E293B);
                ...
                draw_consolas_bold_text(&mut canvas, &mode_desc, prev_x + 20, prev_y + 18, 0x38BDF8, 13.0);
                ...
                fill_rect(&mut canvas, sel_x, sel_y, sel_w, sel_h, 0x0F172A);

                for b in 0..border_w {
                    draw_border(&mut canvas, sel_x - b, sel_y - b, sel_w + b * 2, sel_h + b * 2, 0x00A2FF);
                }
                ...
                fill_rect(&mut canvas, badge_x, badge_y, badge_w, badge_h, 0x1E293B);
                let dim_text_y = (badge_y as i32 + dim_pad_v as i32 + text_off_y).max(0) as usize;
                draw_consolas_bold_text(&mut canvas, &dim_text, badge_x + dim_pad_h, dim_text_y, 0xFFFFFF, font_sz);
                ...
                for (idx, (label, icon_t, icon_sz, bg_normal, bg_hover)) in buttons_data.iter().enumerate() {
                    ...
                    fill_rect(&mut canvas, draw_x, draw_y, draw_w, draw_h, bg_col);
                    ...
                    draw_svg_icon(&mut canvas, *icon_t, icon_x, icon_center_y, *icon_sz);
                    ...
                    if max_text_x > text_x && t_expand > 0.05 {
                        draw_consolas_bold_text_clipped(&mut canvas, label, text_x, text_y, max_text_x, 0xFFFFFF, font_sz);
                    }
                ...
                // Bottom code bar
                fill_rect(&mut canvas, 15, buf_h - 45, buf_w - 30, 36, 0x020617);
                draw_border(&mut canvas, 15, buf_h - 45, buf_w - 30, 36, 0x38BDF8);
                ...
                draw_consolas_bold_text(&mut canvas, &code_str, 25, buf_h - 32, 0xFACC15, 12.5);
